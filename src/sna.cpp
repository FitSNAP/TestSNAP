...
void SNA::compute_yi() {
  ...
  parallel_for(
      num_atoms * idxz_max, KOKKOS_CLASS_LAMBDA(const int iter) {
        ...
        //    Atomic updates to ulist due to parallelization over the bispectrum
        //    coeffients.
              ...
              if (iter < nTotal) {
                ...
                for (int k = 0; k < 3; ++k) {
                  ...
                  for (int j = 1; j <= twojmax; j++) {
                    ...
                    // Update dedr with the first element of ulist and ylist
                    // only once with j==1 .
                    if (j == 1) {

                      Kokkos::single(Kokkos::PerThread(team_member), [&]() {
                        dedr(natom, nbor, k) += (dsfac * ubuf2[tid_index].re *
